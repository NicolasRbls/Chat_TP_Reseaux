...
func _ready():
	...
	print("Network: Ready function called")

onready var CS = preload("res://ChatScene.tscn").instance()

func join_server(ip, port, pseudo):
	print(port)
	print("Trying to join server with IP:", ip, "Port:", port, "Pseudo:", pseudo)
	...
	print("Join server function finished")

func send_pseudo(pseudo):
	print("Sending pseudo:", pseudo)
...
func send_message_to_server(mess: Message):
	print("Sending message to server:", mess)
